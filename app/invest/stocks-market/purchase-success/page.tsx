...
function StocksMarketSuccessContent() {
    ...
    <div className="min-h-screen flex flex-col bg-background-light dark:bg-background-dark max-w-[430px] mx-auto">
      ...
      <main className="flex-1 flex flex-col px-6">
        ...
        <div className="mt-10 flex flex-col items-center text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-base">{marketStock.name} ({marketStock.symbol})</p>
        </div>
          ...
          <div className="glass-card rounded-3xl p-6 shadow-xl relative z-10 overflow-hidden">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-slate-700/50">
                <span className="text-sm text-slate-500">Shares Purchased</span>
                <span className="text-sm font-semibold text-premium-teal dark:text-slate-200">{shares.toFixed(5)} {marketStock.symbol}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-slate-700/50">
                <span className="text-sm text-slate-500">Asset Price</span>
                <span className="text-sm font-semibold text-premium-teal dark:text-slate-200">${marketStock.price.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-slate-700/50">
                <span className="text-sm text-slate-500">Date</span>
                <span className="text-sm font-semibold text-premium-teal dark:text-slate-200">{dateStr}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-slate-700/50">
                <span className="text-sm text-slate-500">Transaction ID</span>
                <div className="flex items-center gap-1">
                  <span className="text-sm font-semibold text-premium-teal dark:text-slate-200">GF-8842-X9L0</span>
                  <button onClick={() => navigator.clipboard.writeText('GF-8842-X9L0')}>
        ...
        <div className="mt-auto mb-8 space-y-4">
          ...
          <button
            onClick={() => router.push('/invest/stocks-market')}
...
export default function StocksMarketPurchaseSuccessPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex flex-col bg-background-light dark:bg-background-dark max-w-[430px] mx-auto items-center justify-center"><div className="animate-pulse text-primary font-bold">Loading...</div></div>}>
      <StocksMarketSuccessContent />
    </Suspense>
  )
}
