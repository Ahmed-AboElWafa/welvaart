...
function SendGoldReviewContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { portfolio, updatePortfolio, addTransaction } = useApp()
  ...
  const usdValueNum = amountNum * GOLD_PRICE_PER_GRAM
  
  const handleConfirm = async () => {
    if (!portfolio || amountNum <= 0) return
    setIsProcessing(true)
    try {
      // Deduct from portfolio gold
      await updatePortfolio({ ...portfolio, gold: portfolio.gold - usdValueNum })
      ...
      await addTransaction({
        ...
        type: 'gold',
        description: 'Sent Gold',
        details: `${amountNum.toFixed(2)} g → ${rHandle}`,
        amount: -amountNum,
        currency: 'g',
        value: -usdValueNum
      })

      router.push(`/transfer/send-gold/success?amount=${amountNum}&name=${encodeURIComponent(rName)}&handle=${encodeURIComponent(rHandle)}`)
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col max-w-[430px] mx-auto bg-background-light dark:bg-background-dark shadow-2xl overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col px-4">
        ...
        <div className="flex flex-col items-center mt-4">
          ...
          <div className="flex flex-col items-center justify-center mt-4 space-y-1">
            <p className="text-slate-900 dark:text-slate-100 text-xl font-bold leading-tight tracking-tight">
              {rHandle}
            </p>
            <p className="text-slate-500 text-sm font-medium">{rName}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center mt-8 mb-8 bg-slate-50 dark:bg-slate-800/50 rounded-xl py-8 px-6 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
          <div className="flex items-baseline gap-2">
            <h1 className="text-slate-800 dark:text-white tracking-tighter text-4xl font-extrabold">{amountNum.toFixed(2)}</h1>
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <h2 className="text-slate-500 text-xl font-semibold tracking-tight">≈ ${usdValueNum.toFixed(2)} USD</h2>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 px-2">
          ...
          <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <span className="text-slate-600 dark:text-slate-400 font-medium text-sm">Transaction Fee</span>
            </div>
            <span className="text-slate-900 dark:text-slate-100 font-bold text-sm">Zero Fees</span>
          </div>
      ...
      <footer className="p-6 flex flex-col gap-4 mt-auto">
        ...
        {/* <p className="text-slate-500 text-[10px] text-center px-8 mt-2">
          By tapping Send Gold, you agree to our Terms of Service and acknowledge the current market volatility of
          precious metals.
        </p> */}
      </footer>
    </div>
...
export default function SendGoldReviewPage() {
  return (
