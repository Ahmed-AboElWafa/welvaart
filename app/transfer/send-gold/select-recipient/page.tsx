...
export default function SendGoldSelectRecipientPage() {
      ...
      <main className="flex-1 px-4" style={{ paddingBottom: '32px' }}>
        ...
        <div className="flex items-center justify-center mb-6">
          <div className="inline-flex items-center gap-2 bg-primary/5 border border-primary/10 px-4 py-2 rounded-full">
            <span className="material-symbols-outlined text-accent text-sm">token</span>
        ...
        <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 transition-all">
          <button
            ...
            className={`flex items-center p-5 w-full text-left group transition-colors ${activeMethod === 'welvaart' ? 'bg-primary/5 border-primary' : 'bg-[#F8FAFC] hover:bg-slate-50'}`}
          >
          ...
          {activeMethod === 'welvaart' && (
            <div className="px-5 pb-5 pt-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                onClick={() => router.push(`/transfer/send-gold/enter-amount?name=Welvaart%20User&handle=${encodeURIComponent(gfIdentifier)}&avatar=`)}
        ...
        <div className="flex flex-col gap-2">
          ...
          {contacts.map((contact) => (
            <div
              key={contact.handle}
              onClick={() => router.push(`/transfer/send-gold/enter-amount?name=${encodeURIComponent(contact.name)}&handle=${encodeURIComponent(contact.handle)}&avatar=${encodeURIComponent(contact.avatar)}`)}
