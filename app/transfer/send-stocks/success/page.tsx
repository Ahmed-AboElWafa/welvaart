...
function SendStocksSuccessContent() {
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 flex flex-col items-center">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
            The shares are now available in the recipient's account.
          </p>
        </div>
        ...
        <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-xl p-6 shadow-2xl relative overflow-hidden border border-slate-100 dark:border-slate-700">
          ...
          <div className="text-center mb-8 pt-2">
            <p className="text-slate-400 text-lg font-bold uppercase tracking-widest mb-1">Amount Sent</p>
            <div className="flex items-center justify-center gap-2">
              <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-500 text-lg font-bold">{symbol}</span>
              <span className="h-1 w-1 rounded-full bg-slate-300" />
              <p className="text-slate-500 text-lg font-medium">{parseFloat(shares).toFixed(4)} Shares</p>
            </div>
            <p className="text-primary text-sm font-semibold mt-1">≈ ${parseFloat(usd).toFixed(2)} USD</p>
          </div>
...
export default function SendStocksSuccessPage() {
  return (
